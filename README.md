...
This is **opt-in and School-workspace-only by design** — it exists for recess go/no-go decisions at a school deployment, not for the Home Pi. Leaving `LIGHTNING_SENSOR_ENABLED` unset in `.env` means `node_helper.js` never connects to a broker, and `LightningStrikeCard` isn't part of the shared `config/modes.json` at all — a deployment that wants it adds `"LightningStrikeCard"` to its own (gitignored) `config/school.json`.

**The sensor lives on a remote ESP32, not the Pi.** An earlier version of this feature wired the AS3935 directly to the Pi's own SPI pins, but a board sitting that close to a Pi 4 (and later, a TV) turned out to be an unworkably noisy RF environment no amount of register tuning fixed — see git history for `lib/LightningSensorClient.js` and `scripts/lightning/` if you want the full story. The sensor now sits on its own ESP32 ("LightningNode," `arduino/LightningNode/LightningNode.ino`), physically decoupled from any switching power supply in the house, and reports in over WiFi/MQTT instead.

**How it works:** each ESP32 node runs the AS3935 SPI driver itself (SparkFun AS3935 Lightning Detector Arduino library), publishes one hand-built JSON message per IRQ to `<prefix>/<node>/event` and a retained `"online"`/`"offline"` string to `<prefix>/<node>/status` (the latter doubling as an MQTT Last Will, so the broker marks a node offline itself if it drops without warning) on an MQTT broker (Mosquitto), and `node_helper.js` subscribes to both via `lib/LightningMqttClient.js`. Only `type: "lightning"` events reach the frontend as `NEXUS_LIGHTNING_STRIKE` — `disturber`/`noise` events are expected background chatter and are filtered out in `node_helper.js` (logged at a throttled rate, never broadcast). A `distance_km` of `63` (the AS3935's "out of range/overhead" sentinel) is normalized to `null` rather than shown as a literal distance. Multiple nodes can publish under the same prefix (e.g. `attic` at home, `school-office` at school); `sensorConfirmed` just means "at least one node has been heard from recently," and goes back to `false` if every node goes stale (`LIGHTNING_MQTT_STALE_MS`, default 150s — 2.5x the firmware's 60s heartbeat interval, since there's no "process exited" signal over MQTT the way there was for a spawned daemon).
...
**Config:** set `LIGHTNING_SENSOR_ENABLED=true` and `LIGHTNING_MQTT_URL` (e.g. `mqtt://server1:1883`) in `.env`; `LIGHTNING_MQTT_USERNAME`/`LIGHTNING_MQTT_PASSWORD` for a broker that requires auth; `LIGHTNING_MQTT_TOPIC_PREFIX` (default `nexus/lightning`) and `LIGHTNING_MQTT_STALE_MS` (default `90000`) to override the defaults. `LightningStrikeCard` itself reads `lightningSensor.distanceThresholdKm` (default `15`) and `lightningSensor.activeWindowMinutes` (default `30`) via `ConfigManager` — how close and how recent a strike has to be to show as active.
...
| `LIGHTNING_SENSOR_ENABLED` / `LIGHTNING_MQTT_URL` / `LIGHTNING_MQTT_USERNAME` / `LIGHTNING_MQTT_PASSWORD` / `LIGHTNING_MQTT_TOPIC_PREFIX` / `LIGHTNING_MQTT_STALE_MS` | AS3935 lightning sensor (remote ESP32 over MQTT), School workspace only (optional — leave `LIGHTNING_SENSOR_ENABLED` blank to disable; see [Lightning Sensor](#lightning-sensor-school-workspace-only)) |
...
- The AS3935 lightning sensor (see [Lightning Sensor](#lightning-sensor-school-workspace-only)) now runs on a remote ESP32 over MQTT rather than wired to the Pi. `lib/LightningMqttClient.js`'s connect/subscribe/parse logic (including the `distance_km === 63` normalization and the online/offline status parsing) is confirmed against `arduino/LightningNode/LightningNode.ino`'s actual publish code, and confirmed working end-to-end against a real ESP32 node publishing real `disturber` events and a real retained `offline` status to a live Mosquitto broker. A genuine `type: "lightning"` event and the staleness watchdog actually tripping on a real dead node have not been exercised yet — worth watching the console the first time a real storm (or a manual antenna tap) comes through.
