...
class CwopClient {
    ...
    pushOnce() {
        ...
        const socket = net.createConnection({ host: this.host, port: this.port }, () => {
            ...
            socket.write(`${packet}\r\n`);
            sent = true;
            // The write() calls above hand the packet to the OS socket
            // buffer synchronously, which is the actual point of delivery -
            // CWOP's servers don't send a meaningful application-level ack,
            // so there's nothing further to confirm. Log success right here
            // rather than waiting for `close`: this network path's TCP
            // teardown with cwop.aprs.net routinely takes longer than our
            // 10s idle timeout (observed even when calling end() immediately
            // after writing), which used to make a fully-successful push
            // look like a failure.
        ...
        socket.on("timeout", () => {
            // By this point the packet has already been handed to the OS
            // (or never will be - see the "before" case below); this is
            // just slow/stuck connection teardown, not a delivery failure.
            if (!sent) {
                console.warn("[Nexus CWOP] Connection to CWOP server timed out before the packet could be sent.");
            }
            socket.destroy();
        });
