...
class RtlWeatherClient {
     ...
     * @param {number} staleTimeoutMs - how long to go without a decoded
     *   reading before considering the sensor offline. Covers both a
     *   crashed/respawning process AND a process that's still running fine
     *   but hearing nothing (dead battery, sensor moved out of RF range) -
     *   neither case produces a process-level event on its own, so this is
     *   a reading-driven watchdog rather than tied to process exit.
     */
    constructor({ command = "rtl_433", args = ["-f", "915M", "-F", "json"], deviceId = null, restartDelayMs = 5000, staleTimeoutMs = 90000 } = {}) {
        ...
        this.onUpdate = null;
        // Fired when staleTimeoutMs elapses with no decoded reading - the
        // caller's cue to mark the station offline until onUpdate resumes.
        this.onStale = null;
        // Fired for every decoded Ecowitt WH31 fridge/freezer sensor
        // reading (model "AmbientWeather-WH31B"), heard on this same
        // rtl_433 stream alongside the Vevor-7in1 station. Kept as a
        // separate callback rather than folded into onUpdate - it's an
        // unrelated sensor family with its own per-channel identity, not
        // another field on the weather station reading.
        this.onFridgeSensorUpdate = null;
        this.proc = null;
        this.buffer = "";
        this.stopped = false;
    ...
    _resetWatchdog() {
        ...
        this.watchdogTimer = setTimeout(() => {
            console.warn(`[Nexus rtl_433] No reading in ${this.staleTimeoutMs}ms - sensor considered offline`);
    ...
    _handleLine(line) {
        ...
        // Ecowitt WH31 fridge/freezer sensors decode as this model on the
        // exact same rtl_433 stream - handled in parallel with, not instead
        // of, the Vevor-7in1 station logic below (neither branch touches
        // the other's fields or the station watchdog).
        if (msg.model === "AmbientWeather-WH31B") {
            if (this.onFridgeSensorUpdate) this.onFridgeSensorUpdate(msg);
            return;
        }
